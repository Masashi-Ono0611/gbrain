...
| `claude-cli exited <code>: ...` | Non-zero exit from the `claude` subprocess where stdout carried no parseable result envelope; the message is whatever the CLI wrote to stderr/stdout | Run `claude` interactively with the same model to see the underlying CLI error directly (e.g. not logged in, model unavailable) |
