 ...
 * tokens are a subset of input_tokens, not additional) — for those, cache
 * classes are priced at $0 and cache reads are subtracted from
 * input_tokens before pricing (cache-as-zero): a true lower bound under
 * both subset and disjoint semantics, instead of nulling the whole row
 * (which hid ~60% of actual OpenAI billing on cache-heavy nightly runs).
 */
 ...
 *
 * INVARIANT (pinned by the 'non-anthropic model: cache rates null' test):
 * non-Anthropic providers must keep BOTH cache rates null. computeCostUsd
 * uses "cache rate is null" as its only signal for subset-style cache
 * accounting — verifying a non-Anthropic cache rate here without adding an
 * explicit subset/disjoint flag would double-count subset-semantics input.
 */
 ...
 * fields contribute nothing (they are unobserved, not free). A nonzero
 * cache class without a verified rate is priced at $0 — and cache reads
 * are additionally subtracted from input_tokens (clamped at 0) because
 * some providers (OpenAI) report them as a subset of input. Under subset
 * semantics the remainder prices tight; under disjoint semantics the
 * subtraction only loosens the bound downward — either way cost_usd stays
 * a true lower bound (cache never bills below $0). Rows priced this way
 * are identifiable by rate_snapshot.cache_read_per_mtok === null with
 * cost_usd non-null.
 *
 * Known limitation (pre-existing, unchanged by cache-as-zero): when the
 * provider does not report the cache-read field at all (null), input_tokens
 * prices at the full rate; if hidden cached reads were billed at a discount
 * the input term can exceed actuals. The bound is therefore "true given
 * complete token reporting".
 */
export function computeCostUsd(usage: UsageCounts, rate: RateSnapshot | null): number | null {
  ...
  if (usage.cache_read_tokens !== null && usage.cache_read_tokens > 0) {
    if (rate.cache_read_per_mtok === null) {
      // cache-as-zero: unverified read semantics — $0 for the class, and
      // subtract from input in case the provider reports reads as a subset
      // of input_tokens (OpenAI does). Clamp guards disjoint semantics.
      if (inputTokens !== null) inputTokens = Math.max(0, inputTokens - usage.cache_read_tokens);
    } else {
      cost += (usage.cache_read_tokens / M) * rate.cache_read_per_mtok;
    }
  }
  if (usage.cache_creation_tokens !== null && usage.cache_creation_tokens > 0) {
    if (rate.cache_write_per_mtok !== null) {
      cost += (usage.cache_creation_tokens / M) * rate.cache_write_per_mtok;
    }
    // else cache-as-zero: unverified write semantics contribute $0.
  }
  if (inputTokens !== null) cost += (inputTokens / M) * rate.input_per_mtok;
