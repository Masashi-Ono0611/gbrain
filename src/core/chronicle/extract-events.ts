...
export async function runChronicleExtract(
  ...
  if (sourceId !== 'default') {
    ...
    if (!fileBacked) {
      // `page` (from the scoped getPage call above) does NOT carry
      // `effective_date` — BOTH engines' getPage SELECT projection omits it
      // (codex review round 4: confirmed via postgres-engine.ts/pglite-engine.ts).
      // Fetching it here as its own targeted query rather than widening the
      // shared getPage projection (which many other callers also use) keeps this
