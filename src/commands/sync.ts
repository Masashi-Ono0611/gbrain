  ...
  if (renamesToDo.length > 0) {
    ...
    if (opts.sourceId) {
      ...
      for (let i = 0; i < fromPaths.length; i += DELETE_BATCH_SIZE) {
        ...
        // #3583 review (Codex deep-dive): this batched pre-resolve had
        // regressed to a raw `engine.resolveSlugsByPaths` + unguarded
        // `resolveSlugForPath` fallback, silently dropping the #3942
        // foreign-origin guard for the sourceId-set rename lane (the
        // guarded resolver is still used correctly by the sibling delete
        // lane a few hundred lines up). Restored to the guarded resolver
        // that master uses here.
        // #3942: guarded resolver — a refused from-path gets NO entry, so
        // the rename below skips updateSlug and falls back to add + reconcile.
    ...
    for (const { from, to } of renamesToDo) {
      ...
      // #3583 review (Codex deep-dive): both branches here had regressed
      // off the #3942 guard (see the batched pre-resolve above for the
      // sourceId-set lane; this restores the no-sourceId lane too — it had
      // silently swapped to the unguarded `resolveSlugByPathOrSourcePath`,
      // which KEY_FILES.md itself documents as NOT the resolver this call
      // site should use).
