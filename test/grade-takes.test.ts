...
describe('runPhaseGradeTakes — phase integration', () => {
  ...
  test('#3044: a non-rate-limit error between 429s resets the consecutive streak', async () => {
    const takes = [
      buildTake({ id: 1, sinceDate: '2023-01-01' }),
      buildTake({ id: 2, sinceDate: '2023-01-01' }),
      buildTake({ id: 3, sinceDate: '2023-01-01' }),
    ...
    const judge: JudgeFn = async () => {
      calls++;
      // rate_limit, timeout, rate_limit, rate_limit — only the final two are
      // consecutive, so all four takes remain eligible for processing.
      if (calls === 2) throw new Error('judge timeout');
      throw Object.assign(new Error('rate limited'), { status: 429 });
    };
    const result = await runPhaseGradeTakes(buildCtx(engine), { judge });
