...
describe('performSync — SyncDisabledError choke point', () => {
  ...
  // v0.51.0.0 managed sync: runAuthenticatedSyncSlice calls performManagedSync directly (bypassing performSync),
  // so the exclusion is enforced at performManagedSync's entry too. The enabled-source control proves the guard
  // does not over-fire (it fails later with the owner-binding error, not SyncDisabledError).
  test('performManagedSync (called directly) refuses a disabled source; an enabled one is not refused', async () => {
    await insertSource('mgddis', '/tmp/mgddis-src', { syncEnabled: false });
    await insertSource('mgdena', '/tmp/mgdena-src', { syncEnabled: true });
    const attempt = async (sourceId: string): Promise<unknown> => {
      try { await performManagedSync(engine, { sourceId, noPull: true, dryRun: true }); return null; } catch (e) { return e; }
    };
    const refused = await attempt('mgddis');
    expect(refused).toBeInstanceOf(SyncDisabledError);
    expect((refused as SyncDisabledError).sourceId).toBe('mgddis');
    expect(await attempt('mgdena')).not.toBeInstanceOf(SyncDisabledError);
  }, 30_000);
