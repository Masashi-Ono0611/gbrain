...
describe('#3583 review: anchor-only liveness can bank a fallback rename duplicate', () => {
  test('anchor-only own old slug is banked without a sentinel until a full sync', async () => {
    const { performSync } = await import('../src/commands/sync.ts');
    const { loadSyncFailures, renameSentinelPath } = await import('../src/core/sync.ts');
    const { checkContentHashDuplicates, checkUndeclaredDbOnlyPages } =
      await import('../src/commands/doctor.ts');
    const liveControlMd = [
      '---', 'type: person', 'title: Live Control', 'slug: live-control', '---',
      '', 'The control still has a tracked backing file.',
    ].join('\n');
    const repo = mkRepo({
      '\u{1F389}.md': exoticMd,
      '\u{1F38A}.md': liveControlMd,
    });
    await performSync(engine, { repoPath: repo, ...SYNC_OPTS });
    expect(await engine.getPage('party-notes')).not.toBeNull();
    expect(await engine.getPage('live-control')).not.toBeNull();

    // Put a genuinely-live control row into the same source_path candidate
    // set. Its own tracked emoji file still derives to live-control TODAY,
    // so the current-tree half of slugLiveness must spare it legitimately.
    await engine.executeRaw(
      `UPDATE pages SET source_path = '\u{1F389}.md'
       WHERE source_id = 'default' AND slug = 'live-control'`,
    );

    // Occupy the ordinary destination so updateSlug cannot take the cheap
    // path. The destination import will overwrite this row with the renamed
    // file's content, activating the fallback-to-add reconcile.
    await engine.putPage('notes/renamed', {
      type: 'person', title: 'Destination occupier', compiled_truth: 'stale destination content',
    }, { sourceId: 'default' });
    mkdirSync(join(repo, 'notes'), { recursive: true });
    execSync('git mv "\u{1F389}.md" notes/renamed.md', { cwd: repo, stdio: 'pipe' });
    writeFileSync(join(repo, 'notes/renamed.md'), [
      '---', 'type: person', 'title: Party Notes', '---',
      '', 'Party notes live here.',
    ].join('\n'));
    execSync('git add -A && git commit -m "rename fallback file to ordinary path"', {
      cwd: repo,
      stdio: 'pipe',
    });

    const renameRun = await performSync(engine, { repoPath: repo, ...SYNC_OPTS });
    expect(renameRun.status).toBe('synced');
    expect((await engine.getPage('notes/renamed'))?.compiled_truth).toContain('Party notes live here.');

    // BUG REPRO: party-notes has no current backing file and is the old row
    // of THIS rename, but the whole-anchor union still labels its old slug
    // live. It survives beside the destination, the bookmark advances, and
    // no rename sentinel tells the next sync to retry the reconcile.
    expect(await engine.getPage('party-notes')).not.toBeNull();
    expect(await engine.getPage('live-control')).not.toBeNull();
    expect(loadSyncFailures().filter(f =>
      f.state === 'open' && f.path === renameSentinelPath('notes/renamed.md')
    )).toHaveLength(0);

    // No rename-specific persistent signal exists, but the broader doctor
    // checks do expose the resulting duplicate/unbacked-page state. This is
    // why the defect is indefinite incremental non-convergence, not absolute
    // invisibility to every operator surface.
    expect((await checkContentHashDuplicates(engine)).status).toBe('warn');
    expect((await checkUndeclaredDbOnlyPages(engine)).status).toBe('warn');

    const quietRetry = await performSync(engine, { repoPath: repo, ...SYNC_OPTS });
    expect(quietRetry.status).toBe('up_to_date');
    expect(await engine.getPage('party-notes')).not.toBeNull();

    // CONTROL for the claimed permanence: full-sync purge is a real cleanup
    // route. Restore the live control's correct bookkeeping first so the
    // purge can distinguish it from the anchor-only stale row.
    await engine.executeRaw(
      `UPDATE pages SET source_path = '\u{1F38A}.md'
       WHERE source_id = 'default' AND slug = 'live-control'`,
    );
    const fullRun = await performSync(engine, { repoPath: repo, ...SYNC_OPTS, full: true });
    expect(fullRun.status).toBe('first_sync');
