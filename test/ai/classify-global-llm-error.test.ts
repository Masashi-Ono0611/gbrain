...
import { classifyGlobalLlmError, normalizeAIError, AIConfigError } from '../../src/core/ai/errors.ts';
...
describe('classifyGlobalLlmError — positives', () => {
  ...
  test('explicit missing-credential AIConfigError messages classify as auth', () => {
...
describe('classifyGlobalLlmError — negatives (per-item errors stay per-item)', () => {
  ...
  test('null for phrase matches inside a --- raw --- output slice (content-dependent halts forbidden)', () => {
    ...
    expect(
      classifyGlobalLlmError(new Error(
        'claude-cli output not JSON: Unexpected token\n--- raw ---\n{"status":401,"answer":"example"}',
      )),
    ).toBeNull();
    expect(
      classifyGlobalLlmError(new Error(
        'claude-cli output not JSON: Unexpected token\n--- raw ---\n{"statusCode":429,"answer":"example"}',
      )),
    ).toBeNull();
  });

  test('generic InvalidArgumentError does not alias to auth', () => {
    const normalized = normalizeAIError(Object.assign(
      new Error('Invalid request options for this item'),
      { name: 'InvalidArgumentError' },
    ));
    expect(normalized).toBeInstanceOf(AIConfigError);
    expect(classifyGlobalLlmError(normalized)).toBeNull();
  });

  test('hostile getters and Proxies fail open without throwing', () => {
    const throwingStatus = new Error('original failure');
    Object.defineProperty(throwingStatus, 'status', {
      get() { throw new Error('hostile status getter'); },
    });
    expect(classifyGlobalLlmError(throwingStatus)).toBeNull();

    const hostileProxy = new Proxy({}, {
      get() { throw new Error('hostile proxy getter'); },
      getPrototypeOf() { throw new Error('hostile proxy prototype'); },
    });
    expect(classifyGlobalLlmError(hostileProxy)).toBeNull();
  });
