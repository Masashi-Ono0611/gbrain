...
describe('computeCostUsd', () => {
  ...
  test('cache tokens present but cache rate unverified: cache-as-zero lower bound, not null', () => {
    const noCache = { ...sonnet, cache_read_per_mtok: null, cache_write_per_mtok: null };
    const M = 1_000_000;
    // reads subtract from input (subset semantics) and price the class at $0
    expect(
      computeCostUsd({ input_tokens: 100, output_tokens: 100, cache_read_tokens: 5, cache_creation_tokens: 0 }, noCache),
    ).toBeCloseTo((95 / M) * noCache.input_per_mtok + (100 / M) * noCache.output_per_mtok, 12);
    // creation prices at $0; input NOT reduced (creation is never an input subset we can verify)
    expect(
      computeCostUsd({ input_tokens: 100, output_tokens: 100, cache_read_tokens: 0, cache_creation_tokens: 5 }, noCache),
    ).toBeCloseTo((100 / M) * noCache.input_per_mtok + (100 / M) * noCache.output_per_mtok, 12);
    // disjoint-semantics guard: reads exceeding input clamp the input term at 0
    expect(
      computeCostUsd({ input_tokens: 100, output_tokens: 0, cache_read_tokens: 200, cache_creation_tokens: 0 }, noCache),
    ).toBeCloseTo(0, 12);
    // zero cache tokens: the unverified cache rate is irrelevant — priced in full.
    expect(
      computeCostUsd({ input_tokens: 100, output_tokens: 100, cache_read_tokens: 0, cache_creation_tokens: 0 }, noCache),
    ).toBeCloseTo((100 / M) * noCache.input_per_mtok + (100 / M) * noCache.output_per_mtok, 12);
  });

  test('cache_read field unreported (null): input prices in full — documented pre-existing looseness', () => {
    const noCache = { ...sonnet, cache_read_per_mtok: null, cache_write_per_mtok: null };
    const M = 1_000_000;
    expect(
      computeCostUsd({ input_tokens: 100, output_tokens: 0, cache_read_tokens: null, cache_creation_tokens: 0 }, noCache),
    ).toBeCloseTo((100 / M) * noCache.input_per_mtok, 12);
  });

  test('mixed rates (read verified, write not — anthropic invalid TTL): read exact, creation $0', () => {
    const mixed = { ...sonnet, cache_write_per_mtok: null };
    const M = 1_000_000;
    expect(
      computeCostUsd({ input_tokens: 100, output_tokens: 100, cache_read_tokens: 50, cache_creation_tokens: 70 }, mixed),
    ).toBeCloseTo(
      (100 / M) * mixed.input_per_mtok + (100 / M) * mixed.output_per_mtok + (50 / M) * (mixed.cache_read_per_mtok as number),
      12,
    );
  });
