...
describe('runPhaseProposeTakes — global-error halt (#3044)', () => {
  ...
  test('a non-rate-limit error between 429s resets the consecutive streak', async () => {
    const pages = [
      buildPage({ slug: 'wiki/a', body: 'page a prose' }),
      buildPage({ slug: 'wiki/b', body: 'page b prose' }),
      buildPage({ slug: 'wiki/c', body: 'page c prose' }),
    ...
    const extractor: ProposeTakesExtractor = async () => {
      extractorCalls++;
      // rate_limit, timeout, rate_limit, rate_limit — only two consecutive
      // rate-limit hits at the end, so the phase must not claim three.
      if (extractorCalls === 2) throw new Error('extractor timeout');
      throw Object.assign(new Error('rate limited'), { status: 429 });
    };
    const result = await runPhaseProposeTakes(buildCtx(engine), { extractor });
    ...
    expect(details.aborted_global_error).toBeUndefined();
    expect(result.status).toBe('warn');
    expect(result.summary).not.toContain('consecutive rate_limit errors');
  });
